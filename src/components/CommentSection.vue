      ...
      <div class="card-body">
        ...
        <div v-if="comments.length" class="comments-section">
          ...
          <ul v-show="commentsVisible" class="list-unstyled">
            <li v-for="(comment, index) in comments" :key="comment.id" class="mb-3 comment-item">
              <div class="comment-header d-flex align-items-center mb-2">
                <div class="comment-username">{{ comment.username || '匿名用户' }}</div>
                <div class="comment-time ms-3">{{ formatDate(comment.timestamp) }}</div>
                <button class="btn btn-outline-success btn-sm ms-auto" @click="likeComment(comment.id, index)">
                  <i class="bi bi-hand-thumbs-up-fill"></i> {{ comment.likes }}
                  <span class="like-text">点赞</span>
                </button>
              </div>
              <div class="bg-light p-3 rounded shadow-sm comment-content">{{ comment.content }}</div>
            </li>
          </ul>
          <button @click="toggleComments" class="btn toggle-comments-btn mt-2">
            {{ commentsVisible ? '折叠评论' : '展开评论' }}
            <i :class="commentsVisible ? 'bi bi-chevron-up' : 'bi bi-chevron-down'"></i>
          </button>
        </div>
      </div>
    </div>
